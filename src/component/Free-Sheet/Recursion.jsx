...
function Recursion() {
  ...
  useEffect(() => {
    const fetchUserArrayQuestions = async () => {
      try {
        ...
        if (user) {
          const result = await service.RecursionQuestions(user.$id);
        ...
        console.error('Error fetching user or questions Recursion:', error);
      }
      finally {
        setLoading(false)
      }
  ...
  const handleCheckboxChange = async (event, questionId, completed) => {
    ...
    try {
      await service.updateRecursionQuestionStatus(questionId, completed);
